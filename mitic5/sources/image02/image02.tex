...
Dès que vous avez ouvert un nouveau fichier dans \emph{Gimp}, pensez à le sauvegarder au format \texttt{Nom-seance1.png} : dans le menu \texttt{Fichier}, choisir \texttt{Enregistrer}. Pendant que vous travaillez, pensez à sauvegarder régulièrement votre travail (raccourci clavier \texttt{Cmd + s}).   
...
Intégrer votre travail dans un fichier \emph{Word} en tant que lettrine de votre texte (c'est à dire que l'image sera entourée de texte en bas et à droite). 
...
Dès que vous avez ouvert un nouveau fichier dans \emph{Gimp}, pensez à le sauvegarder au format \texttt{Nom-seance2.png} : dans le menu \texttt{Fichier}, choisir \texttt{Enregistrer}. Pendant que vous travaillez, pensez à sauvegarder régulièrement votre travail (raccourci clavier \texttt{Cmd + s}).   
...
\item Ouvrir une de ces images sous \emph{Gimp}.
\item Créer deux nouveaux calques (leur donner un nom explicite) et y copier les deux autres images : vous devez alors avoir sous \emph{Gimp} une image composée de trois calques qui contiennent chacun une image fournie par l'enseignant.
...
\end{enumerate}
\vspace{10pt}
Une fois le travail terminé, vous exporterez l'image au format PNG (le fichier doit être nommé à partir de votre nom : \texttt{Nom-seance2.png}), puis vous le rendrez sur \emph{Teams} à l'endroit indiqué par votre enseignant (si nécessaire, se reporter à la fiche méthode \emph{Remettre son devoir}, page \pageref{TeamsRemettreDevoir}).}
% fin de l'énoncé
...
\begin{itemize}   
\item \textbf{ajouter un texte sur une image}, voir section \vref{Gimp2AjouterTexte} ;
\item \textbf{ajouter un filtre sur une portion de l'image}, voir section \vref{Gimp2AppliquerFiltre} ;
\item \textbf{convertir une image en noir et blanc (niveaux de gris)}, voir section \vref{Gimp2ConvertirNoirBlanc} ;
\item \textbf{travailler avec les calques}, voir section \vref{Gimp2Calques} ;   
\item \textbf{réaliser une capture d'écran}, voir section \vref{CaptureEcran2}.
\end{itemize}  
...
Dans \emph{Gimp} il est possible de travailler avec des \emph{calques} qui correspondent à des couches successives que l'on ajoute sur l'image de base. L'image finale correspond alors à la superposition de tous les calques. Toute la gestion des calques se passe dans la fenêtre des calques.
