\chapter{Programmation Scratch 2}  
...
\section*{Passer Scratch en langue française} 
...
\section{Séance 1 : un programme qui dessine une spirale}\label{ficheScratch5e1}


\boiteEnonceLarge{Le but de cette séance est de dessiner une spirale, comme montré sur la figure ci-dessous. Une fois cette première étape achevée, il faudra dessiner une fleur en réutilisant le script conçu pour dessiner la spirale.}

\uneimageici{./images/scratch02/1ResultatSpirale}{.4\textwidth}


\subsection{Avant de commencer...}

Donnez un nom à votre programme pour le sauvegarder : dans le menu \texttt{Fichier}, choisir \texttt{Enregistrer}. Pendant que vous travaillez, pensez à sauvegarder régulièrement votre travail (raccourci clavier \texttt{Cmd + S}).   


\subsection{Première étape : création d'une variable}\label{Scratch5eCreationVariable}\index{Scratch!Créer une variable}\index{Créer une variable (Scratch)} 
...
\subsection{Deuxième étape : le script qui dessine la spirale} 
...
\subsection{Troisième étape : un script qui efface l'écran}\index{Scratch!Effacer l'écran}\index{Effacer l'écran (Scratch)}
...
\subsection{Quatrième étape : à vous de jouer !}
...
\subsection{Rendre le programme}

Une fois le programme terminé, dans le menu \texttt{Fichier} choisir \texttt{Enregistrer} (le fichier doit être nommé à partir de votre nom : \texttt{Nom-Prénom-date.sb2}) et le rendre sur la plateforme Moodle à l'endroit indiqué par votre enseignant (si nécessaire, se reporter à la fiche méthode \emph{Remettre un devoir sur Moodle}, section \vref{MoodleRendreDevoir}).
...
Aide : on peut créer une variable \emph{PositionY} qui contient la position en $y$ du lutin, puis l'utiliser dans une boucle qui se répète tant que \emph{PositionY} n'a pas atteint la valeur souhaitée. Avant de terminer la boucle, il faudra bien entendu modifier la valeur de \emph{PositionY}.
...
Par exemple, le programme pourra poser la question \emph{<<\,Bonjour, combien font $10 \times (-5)$ ?\,>>}. L'utilisateur devra alors répondre $-50$ (cette réponse est entrée au clavier). La figure ci-dessous montre différentes étapes de l'exécution du programme.}
...
\subsection{Algorithme du programme}  

L'algorithme du programme est le suivant :

\uneimageici{./images/scratch02/flowchartActivite2}{.8\textwidth}


\subsection{Aide pour l'écriture du programme}  
...
\subsection{Première étape : création de l'arrière plan et choix des lutins} 
...
\subsection{Définition du mouvement de la balle} 
...
\subsection{Mouvements de la raquette et rebond de la balle sur la raquette}
