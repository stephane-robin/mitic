...
Dès que vous avez ouvert un nouveau document dans \emph{PowerPoint}, sauvegardez-le au format \texttt{Nom-seance1.pptx} : dans le menu \texttt{Fichier}, choisir \texttt{Enregistrer sous}.
...
Insérer une vidéo dans une diapositive. Préparer ensuite des notes relatives au contenu de votre présentation, afin de pouvoir utiliser cette présentation devant la classe.
...
\subsubsection{Insérer une zone de texte dans une diapositive}\index{Impress!Insérer un texte}\index{Texte, insérer (Impress)}\label{Presentation1texte}
...
\subsubsection{Insérer une image directement dans une diapositive}\index{Impress!Insérer une image}\index{Image, insérer (Impress)}\label{Presentation1image}
