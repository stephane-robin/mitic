...
Sur la page \emph{Fichiers} de Teams, récupérer le fichier \texttt{LaBelleEtLaBete.docx}. Si nécessaire, se reporter à la fiche méthode  \emph{Consulter et télécharger un document} sur \emph{Teams} à la page \pageref{teamsConsulterTelecharger}.\\
...
Dès que vous avez ouvert un nouveau document dans \emph{Excel}, sauvegardez-le au format \texttt{Nom-seance2.docx} : dans le menu \texttt{Fichier}, choisir \texttt{Enregistrer sous}.
...
\end{itemize}
\vspace{12pt}
Vous pouvez également rajoutez à votre document un en-tête et un pied de page mentionnant le numéro de page et le titre de l'oeuvre.
...
\begin{center}
\qrcode[hyperlink, height=0.9in]{https://web.microsoftstream.com/video/6a77d6c6-3fd6-4a71-a1e1-0d0b4ebdd050}
\end{center}
...
Dès que vous avez ouvert un nouveau document dans \emph{Excel}, sauvegardez-le au format \texttt{Nom-seance3.docx} : dans le menu \texttt{Fichier}, choisir \texttt{Enregistrer sous}.
