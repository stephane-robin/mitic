...
%\section*{Les années précédentes, vous avez appris...} %\hfill \includegraphics[width=3cm]{./images/generales/VuEn6e}
...
\begin{center}
\qrcode[hyperlink, height=0.9in]{https://web.microsoftstream.com/video/61bea4db-d1c2-41c7-8312-67b66c431666}
\end{center}
...
\item géographie, démographie, climat, principales villes
\item symboles et devises
\item culture et traditions
\item régime politique
\item sports
\item personnalités célèbres
\item pourquoi ai-je choisi ce pays ?
\end{enumerate}
} % fin du emph
\vspace{10pt}
...
\end{itemize}
\vspace{10pt}
Une fois la mise en forme terminée, vous exporterez votre fichier au format DOCX (le fichier doit être nommé à partir de votre nom : \texttt{Nom-seance1.docx}), puis vous le rendrez sur \emph{Teams} à l'endroit indiqué par votre enseignant (si nécessaire, se reporter à la fiche méthode \emph{Remettre son devoir}, page \pageref{TeamsRemettreDevoir}).
}% fin de l'énoncé
...
\begin{center}
\qrcode[hyperlink, height=0.9in]{https://web.microsoftstream.com/video/824738cb-136b-4966-ab0e-cf85b387cd41}
\end{center}
...
\boiteEnonceLarge{Le but de cette séance est de mettre en forme une fiche de lecture à l'aide du traitement de texte \emph{Microsoft Word}. La structure de la fiche de lecture vous sera communiquée par votre enseignant de français.}
\vfill
\phantom{rien}
\boiteEnonceLarge{
Pour cela, vous devrez :
...
\end{itemize}
\vspace{10pt}
Une fois la mise en forme terminée, vous exporterez votre fichier au format DOCX (le fichier doit être nommé à partir de votre nom : \texttt{Nom-seance2.docx}), puis vous le rendrez sur \emph{Teams} à l'endroit indiqué par votre enseignant (si nécessaire, se reporter à la fiche méthode \emph{Remettre son devoir}, page \pageref{TeamsRemettreDevoir}).
...
\begin{center}
\qrcode[hyperlink, height=0.9in]{https://web.microsoftstream.com/video/69f2a181-e7a8-4e18-ad92-72b4f8cd711d}
\end{center}
...
\boiteEnonceLarge{Le but de cette séance est de rédiger un document mathématique à l'aide du traitement de texte \emph{Microsoft Word}. Vous trouverez ci-dessous le contenu de ce document :\\

\textbf{1) Définition}\\

La racine carrée du nombre positif $a$ est le nombre \emph{positif} $x$ tel que $x^2=a$.\\

On note $x=\sqrt{a}$\\

Donc, pour tout nombre $a\geq 0$: 
...
a) Pour tous  nombres $a$ et $b$ positifs, $\sqrt{a \times b}=\sqrt{a} \times \sqrt{b}$\\

\emph{Exemple :} $\sqrt{27}\times\sqrt{3}=\sqrt{81}=9$\\

b) Pour tous nombres $a$ et $b$ positifs, avec $b > 0$, $\sqrt{\frac{a}{b}}=\frac{\sqrt{a}}{\sqrt{b}}$.\\

\emph{Exemple :} $\frac{\sqrt{27}}{\sqrt{3}}=\sqrt{\frac{27}{3}}=\sqrt{9}=3$.\\

c) Pour tout nombre $a$, $\sqrt{a^2}=a$ ou $\sqrt{a^2}=-a$.\\
...
\left \{
\begin{array}{c}
si \hspace{0.3cm} a\geq 0, \hspace{0.8cm} \sqrt{a^2}=a \\
si \hspace{0.3cm} a\leq 0, \hspace{0.8cm} \sqrt{a^2} = -a \\
\end{array}
\right.
\]

$\sqrt{a^2}$ doit toujours être positif ou nul.\\

\emph{Exemple :} $\sqrt{12^2}=12$ car $12 > 0$ et $\sqrt{(-12)^2}=-(-12)=12$ car $-12 < 0$.\\
...
$(x-3)^2=5 \\ \iff x-3 = \sqrt{5}$ ou $x-3=-\sqrt{5} \\ \iff x=3+\sqrt{5}$ ou $x=3-\sqrt{5}$\\

b) Simplification de radicaux\\

\emph{Exemple :} Simplifier l'écriture des nombres $A=\sqrt{18}$ et $B=5\sqrt{12}+2\sqrt{75}-4\sqrt{27}$ et $C=(1-\sqrt{5})(1+\sqrt{5})$.

$A=\sqrt{9\times 2}=\sqrt{9}\times\sqrt{2}=3\sqrt{2}$\\

$B=5\sqrt{4\times 3}+2\sqrt{25\times 3}-4\sqrt{9\times3}$\\

$B=5\sqrt{4}\times\sqrt{3}+2\sqrt{25}\times\sqrt{3}-4\sqrt{9}\times\sqrt{3}$\\

$B=5\times 2\times\sqrt{3}+2\times5\times\sqrt{3}-4\times3\times\sqrt{3}=10\sqrt{3}+10\sqrt{3}-12\sqrt{3}=8\sqrt{3}$
}% fin enonce
...
\begin{itemize}
%\item sauvegarder le fichier en le nommant à partir de votre nom : \texttt{Nom-date.odt} ; 
\item utiliser des styles pour mettre en forme les titres de chaque section ;
...
Une fois la mise en forme terminée, vous exporterez votre fichier au format DOCX (le fichier doit être nommé à partir de votre nom : \texttt{Nom-seance3.docx}), puis vous le rendrez sur \emph{Teams} à l'endroit indiqué par votre enseignant (si nécessaire, se reporter à la fiche méthode \emph{Remettre son devoir}, page \pageref{TeamsRemettreDevoir}).
}% fin de l'énoncé
...
Intégrer à votre document le graphe de la fonction $f$ définie par $f(x)=x^2$, que vous aurez préalablement construit avec l'application \emph{Geogebra}.
...
\begin{enumerate}
\item Pour attribuer un style à un titre, sélectionner le titre en question \circled{1} puis sur l'onglet \texttt{Mise en forme}  \circled{2} et finalement cliquer sur le menu \texttt{Style...}  \circled{3}
...
puis choisir le style \texttt{Titre 1} \circled{1} et \texttt{Appliquer} \circled{2} la mise en forme.

\uneimageici{./images/texte03/titre2.png}{.9\textwidth}

\item Une fois que vous avez affecté le style \texttt{Titre 1} à tous les titres de paragraphe, pour changer un des attributs de mise en forme des \texttt{Titre 1} \circled{1}, ne sélectionner aucun texte en particulier, mais sélectionner l'onglet \texttt{Mise en forme} puis le menu \texttt{Style...}.

\uneimageici{./images/texte03/titre3.png}{.9\textwidth}

Cliquer maintenant sur \texttt{Modifier...} \circled{2} pour changer chaque élément de style, comme le type de police, la taille ou la couleur par exemple.
...
Si on utilise les styles pour les titres de paragraphes, alors on peut générer une table des matières automatique. Pour cela, positionner le curseur à l'endroit où la table doit être insérée; cliquer sur l'onglet \texttt{Insérer} \circled{1} puis sur \texttt{Table  et index...} \circled{2}
...
Pour ajouter un en-tête ou un pied de page, sélectionner l'onglet \texttt{Insertion} \circle{1} puis \texttt{En-tête} ou \texttt{Pied de page} \circle{2}. 

\uneimageici{./images/texte03/entete1.png}{.9\textwidth}

Dans la partie qui s'ouvre à droite de l'écran, sélectionner le style d'en-tête ou le style de pied de page souhaité.

\uneimageici{./images/texte03/entete2.png}{.4\textwidth}
...
\uneimageici{./images/texte03/entete3.png}{\textwidth}

%\uneimageici{./images/texte03/Texte03_InsereChamp1}{.5\textwidth}

Pour obtenir automatiquement le numéro de page courante et le nombre total de pages du document affichés dans le pied de pages, on peut sélectionner \texttt{Insertion automatique} \circled{2} dans le menu \texttt{Insérer} puis choisir \texttt{Page X sur Y} \circled{3}.

%Ajouter une barre oblique : / puis dans le menu \texttt{Insérer} sélectionner \texttt{Insertion automatique} puis une nouvelle fois \texttt{Insertion automatique...}.

\uneimageici{./images/texte03/numerotation.png}{.46\textwidth}
...
\uneimageici{./images/texte03/indice_exposant.png}{.34\textwidth}

%sur le menu \texttt{format} puis choisir \texttt{caractère}.
...
\uneimageici{./images/texte03/caracteresSpeciaux1.png}{\textwidth}
...
\uneimageici{./images/texte03/caracteresSpeciaux2.png}{.8\textwidth}
...
\uneimageici{./images/texte03/inserer_math1.png}{\textwidth}
...
%\uneimageici{./images/texte03/inserer_math3.png}{.5\textwidth}

%\deuximagesici{./images/texte03/Texte03_Formule4}{\textwidth}{./images/texte03/Texte03_Formule9}{0.5\textwidth}
\deuximagesici{./images/texte03/inserer_math3.png}{\textwidth}{./images/texte03/Texte03_Formule9}{0.5\textwidth}
